...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    const rules = {
      admin : [],
      DSNAdmin : [],
      DSNUser: ["/Axon/MasterKube/general/masterkube/User","/Axon/MasterKube/general/masterkube/Products","/Axon/MasterKube/general/masterkube/Categories","/Axon/MasterKube/general/masterkube/Distributors","/Axon/MasterKube/general/masterkube/Inventories","/Axon/MasterKube/general/masterkube/Plants","/Axon/MasterKube/general/masterkube/Dealers"],
      regularuser : ["/Axon/MasterKube/general/masterkube/Vessel","/Axon/MasterKube/general/masterkube/Voyage"],
      vesseluser: ["/Axon/MasterKube/general/masterkube/Voyage"],
      voyageuser: ["/Axon/MasterKube/general/masterkube/Vessel"]
    }
    var loggeduser = req.headers.username 
    var location = parseURL(req.url.slice(1))
    if(loggeduser!=null || loggeduser!= undefined){
      if(rules[loggeduser] == undefined){
        res.writeHead(403, 'Forbidden', cors_headers);
        res.end('Blocked by the proxy.');
        return;
      }
      else{
        for (var i=0;i<rules[loggeduser].length;i++){
          if ((location.path.toLowerCase()).search(rules[loggeduser][i].toLowerCase()) >=0 ) {
            res.writeHead(403, 'Forbidden', cors_headers);
            res.end('Blocked by the proxy.');
            return;
          }
