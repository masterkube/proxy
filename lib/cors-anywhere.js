...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    var user = req.url.match(/\(([^)]+)\)/)[1]
    console.log(req.url)
    console.log("user ",user)
    var extract_length = user.length+3
    console.log("len ",extract_length)
    var location = parseURL(req.url.slice(extract_length));
    console.log("test2",req.url.slice(extract_length))
    console.log("test1",location)

    if (location.path == "/Axon/MasterKube/general/masterkube/Voyage") {
