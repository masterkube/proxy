...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    const rules = {
      admin : ["/Axon/MasterKube/general/masterkube/Voyage","/Axon/MasterKube/general/masterkube/State"],
      user1 : ["/Axon/MasterKube/general/masterkube/Voyage","/Axon/MasterKube/general/masterkube/State","/Axon/MasterKube/general/masterkube/Country","/Axon/MasterKube/general/masterkube/User"]
    }
    var loggeduser = req.headers.username 
    var location = parseURL(req.url.slice(1))
    if(loggeduser!=null || loggeduser!= undefined){
      for (var i=0;i<rules[loggeduser].length;i++){
        console.log(location.path,rules[loggeduser][i])
        if (location.path == rules[loggeduser][i]) {
